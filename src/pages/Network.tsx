      ...
      <Container>
        <div className="py-10">
          <div className="flex items-center gap-3">
            ...
            <div className="text-sm text-slate-700">Add friends and discover friends-of-friends.</div>
          </div>

          <div className="mt-6 grid lg:grid-cols-3 gap-4">
            <div className="card">
              <div className="font-semibold flex items-center gap-2"><UserPlus size={18} /> Add a friend</div>
              <div className="mt-3 text-sm text-slate-700">Send a friend request by email.</div>
            ...
            <div className="card lg:col-span-2">
              ...
              <div className="mt-4 space-y-3">
                {incoming.length === 0 ? (
                  <div className="text-sm text-slate-700">No pending requests.</div>
                ) : (
                  incoming.map((r) => (
          ...
          <div className="mt-6 grid lg:grid-cols-2 gap-4">
            <div className="card">
              ...
              <div className="mt-4 space-y-3">
                {friends.length === 0 ? (
                  <div className="text-sm text-slate-700">No friends yet. Add a friend to start lending.</div>
                ) : friends.map((f) => (
                  <div key={f.uid} className="rounded-2xl border border-slate-200 bg-white p-4 flex items-center justify-between gap-3">
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="text-sm text-slate-600 truncate">{f.email}{f.school ? ` • ${f.school}` : ''}</div>
                      </div>
                    </div>
            ...
            <div className="card">
              ...
              <div className="mt-2 text-sm text-slate-700">People two hops away — safer than random.</div>

              <div className="mt-4 grid sm:grid-cols-2 gap-3">
                {fof.length === 0 ? (
                  <div className="text-sm text-slate-700">No friends-of-friends found yet.</div>
                ) : fof.map((p) => (
                  <div key={p.uid} className="rounded-2xl border border-slate-200 bg-white p-4 flex items-center gap-3">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="text-xs text-slate-600 truncate">{p.email}</div>
                    </div>
                  </div>
                ))}
              </div>
